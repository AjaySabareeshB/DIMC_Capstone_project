...
\IEEEPARstart{T}{he} rapid deployment of Artificial Intelligence (AI) at the edge has fundamentally shifted the computing landscape, bringing real-time inference to mobile devices, IoT sensors, and autonomous systems. However, traditional Von Neumann architectures suffer from the "memory wall" bottleneck. In these traditional setups, the continuous movement of data between the memory and processing units consumes significantly more energy than the actual computation itself. Digital In-Memory Computing (DIMC) has emerged as a critical solution to this inefficiency. By embedding arithmetic logic directly within or near the memory array, DIMC drastically reduces data shuttling, offering high-throughput and energy-efficient neural network acceleration.

Despite these massive performance benefits, edge AI devices operate in physically exposed environments, making them highly susceptible to physical hardware exploits. Among these, Side-Channel Attacks (SCA) represent a severe threat. SCAs exploit unintentional physical leakages—such as power consumption, electromagnetic emissions, or timing variations—to reverse-engineer the proprietary parameters and weights of deployed AI models. Power Analysis attacks, such as Differential Power Analysis (DPA) and Correlation Power Analysis (CPA), are particularly devastating to DIMC architectures because the parallel activation of memory rows creates a highly correlated, easily measurable power signature.

Securing these edge devices is a paramount challenge, but current state-of-the-art countermeasures often fall into two categories, each with their own drawbacks. Purely physical security techniques, such as switched-capacitor circuits, rely heavily on analog components. The primary vulnerability of these designs is that ensuring these analog components are tuned to prevent any power leakage across all operating conditions is exceptionally difficult and prone to failure. Conversely, purely algorithmic countermeasures, such as Boolean masking, offer provable security guarantees. However, securely implementing both the linear and non-linear masking operations directly in hardware leads to a massive bloat in area overhead—often resulting in a core 12 to 13 times the size of an unprotected baseline. Because of these distinct limitations, there is an urgent need for a hybrid approach. By strategically combining physical security measures with masking-based techniques, we can relax the strict analog tuning requirements while simultaneously bypassing the massive area penalties caused by complex non-linear masking. This hybrid methodology aims to drastically reduce the area overhead of these DIMC cores while maintaining the robust security required for edge deployment.
\begin{figure}
    \centering
...
While the high-parallelism MAC operations are performed within the IMC macro, the non-linear layers—specifically \textbf{Batch Normalization} and \textbf{ReLU}—are executed as \textbf{post-computation} in the Near-Memory Computing (NMC) unit (see Fig. \ref{fig:dimc_core}). By partitioning the architecture this way, we avoid the prohibitive cost of implementing non-linear Boolean masking for the entire pipeline. Instead, we aim to secure these specific near-memory bottlenecks using \textbf{physical security measures}, such as switched-capacitor logic, to decouple the digital switching activity from the $V_{DD}$ supply. This hybrid strategy allows the system to support a full neural network inference flow while targeting an aggregate area overhead of only $5\times$ the unprotected baseline.
...
The mathematical viability of the hybrid masking approach was validated through high-level software modeling. Specifically, the \textbf{XNOR offset correction} methodology was implemented and tested via a Python-based verification script. Simulations confirm that the statistical offsets introduced by substituting standard AND-based multipliers with linear XNOR logic can be fully compensated during the near-memory Batch Normalization phase, ensuring the inference accuracy is not degraded.

\subsection{Hardware Implementation and Synthesis}
At the hardware level, the 3-share Carry Save Adder (CSA) tree for the DIMC array was implemented in Verilog HDL. To quantify the physical footprint of the linearized Boolean masking logic, the design was synthesized using a commercial standard cell library. 

As detailed in Table \ref{tab:synth_results}, the initial synthesis was targeted to a \textbf{TSMC 28nm HPC+} process node. The synthesis report confirms that the CSA tree is purely combinational, requiring 4,176 standard cells and occupying a total cell area of 3,063.56 $\mu\mathrm{m}^2$. This preliminary area extraction establishes the physical baseline for the masked IMC logic, verifying that the linear partial-product summation does not trigger the exponential gate explosion characteristic of masked AND-operations.

\begin{table}[h]
\renewcommand{\arraystretch}{1.3}
\caption{Preliminary Synthesis Results for 3-Share CSA Tree}
...
\hline
\hline
Technology Node & TSMC 28nm HPC+ \\
\hline
Total Cell Count & 4,176 \\
\hline
Combinational Cells & 4,084 \\
\hline
Buffer/Inverter Cells & 324 \\
\hline
\textbf{Total Cell Area} & \textbf{3063.56 $\mu\mathrm{m}^2$} \\
\hline
\end{tabular}
\end{table}
...
The preliminary synthesis of the CSA tree (Table \ref{tab:synth_results}) validates the core hypothesis of our methodology: linearizing the IMC math via XNOR gates bypasses the exponential area bloat of traditional masked multipliers. However, analyzing these results from a physical layout perspective reveals a critical constraint. Extending pure 3-share Boolean masking through the entire multi-bit accumulation phase would introduce severe pitch-matching issues with the memory array columns. The synthesized area confirms that while the Boolean partial-product generation is highly efficient, a strict architectural boundary must be drawn before final accumulation to maintain macro density and viable pitch sizing.

Consequently, ongoing work for the second half of the project is focused on the design and integration of a unified \textbf{Near-Memory Post-Processing Unit}. This peripheral module will aggregate the final multi-bit accumulation, the algorithmic XNOR offset correction, and the Batch Normalization layers into a single pipeline. Because applying Boolean masking to these additive and non-linear stages would violate our $5\times$ area constraint, the security of this post-processing unit relies entirely on \textbf{physical hardware countermeasures}. 

By implementing specialized hardware security—such as switched-capacitor logic—at the near-memory boundary, we aim to decouple the digital switching activity from the main power supply. The ultimate objective is to guarantee \textbf{empirical security} for the entire inference pipeline. Future work will combine this physical layout data with the Project Aurora MLIR compiler to run standardized workloads (e.g., CIFAR-10) through comprehensive Correlation Power Analysis (CPA) simulations, thereby empirically proving the system's SCA resistance against the strict $5\times$ area target.
